...
abstract class CardsRecyclerView(context: Context, attributeSet: AttributeSet? = null, defStyle: Int = 0)
    ...
    fun refresh(newCards: List<Card<*>>) {
        post { (adapter as CardsAdapter).replaceFor(newCards) }
    }
    ...
    protected abstract fun obtainHoldersCreator(): (Int) -> ((ViewGroup) -> CardViewHolder)?

    protected abstract fun launchEntriesLoading()

    protected fun obtainItemDecoration(): ItemDecoration? = null

    protected fun createCardsAdapter() = CardsAdapter(obtainHoldersCreator())
